...
class CondosManagement extends Component {
    constructor() {
        super();
        this.state = {
            managers: {},
    ...
    componentDidMount() {
        database.ref('managementEnterprises').once('value').then(function(snap) {
            snap.forEach(function(childSnap) {
                this.setState({
                    managers: [...this.state.managers, { label: childSnap.child('name').val() + ' - ' + childSnap.child('enterpriseOwner').val(), value: childSnap.child('managementEnterpriseId').val(), status: childSnap.child('status').val(), isCondominiumManager: childSnap.child('isCondominiumManager').val() }]
                });
            }.bind(this));
        }.bind(this));
    ...
    managerValidator(managerId) {
        if(managerId) {
            ...
            return (
                <div className="condos__management-details">
                    <h3>No hay empresa de administración asignada a este condominio</h3>
                </div>
            )
        }
    }
    handleSubmit(e) {
        ...
        database.ref('condos/' + this.props.condoId).child('managementEnterpriseId').set(newManagerId);
        alert('Administrador Asignado');
    }
    render() {
        const { selectedOption } = this.state;
        const data = _.filter(this.state.managers, { 'status': 'active', 'isCondominiumManager': 'true' });
        return (
            <div className="condos__management">
                {this.managerValidator(this.props.managerId)}
                <form onSubmit={(e) => this.handleSubmit(e)}>
                    <div className="condos__input-container">
                        <label>Seleccionar Administración</label>
                        <Select
                    ...
                    <div className="condos__input-container">
                        <button type="submit">Seleccionar Administración</button>
                    </div>
                </form>
            </div>
        )
...
export default CondosManagement;